...
async function syncAppointments() {
  ...
  try {
    ...
    // Debug: log what appointment_type actually looks like in raw Cliniko response
    if (active.length > 0) {
      console.log('  [Debug] appointment_type raw:', JSON.stringify(active[0].appointment_type));
      // Also check lookup keys vs what comes through
      const lookupKeys = Object.keys(apptTypeLookup);
      console.log('  [Debug] lookup has', lookupKeys.length, 'types, sample key:', lookupKeys[0], '-> value:', apptTypeLookup[lookupKeys[0]]);
    }

    // Debug: check if any appointment links to the class appointment type
    const classTypeId = Object.entries(apptTypeLookup).find(([k,v]) => v && v.toLowerCase().includes('class'));
    console.log('  [Debug] Class type in lookup:', classTypeId ? classTypeId[0] + ':' + classTypeId[1] : 'NOT FOUND');
    if (classTypeId) {
      const classMatch = active.find(a => {
        const link = a.appointment_type && a.appointment_type.links && a.appointment_type.links.self ? a.appointment_type.links.self : '';
        return link.includes(classTypeId[0].slice(0, 10)); // partial match to handle any precision issues
      });
      console.log('  [Debug] Appointment with class type found:', classMatch ? 'YES id=' + classMatch.id : 'NO - classes may be in group_appointments endpoint instead');
    }
