...
async function syncAppointments() {
  ...
  try {
    // Fetch regular (non-cancelled) appointments
    const items = await clinikoFetchAll(
      'individual_appointments?per_page=' + PER_PAGE + '&updated_since=' + encodeURIComponent(lastSync),
      'individual_appointments'
    );

    // Fetch cancelled appointments separately — Cliniko stores them at a different endpoint
    console.log('  Fetching cancelled appointments...');
    const cancelledItems = await clinikoFetchAll(
      'individual_appointments?per_page=' + PER_PAGE + '&updated_since=' + encodeURIComponent(lastSync) + '&cancelled=true',
      'individual_appointments'
    );
    console.log('  Cancelled appointments found: ' + cancelledItems.length);

    // Merge both lists
    const allItems = items.concat(cancelledItems);
    console.log('  Total appointments (including cancelled): ' + allItems.length);

    if (allItems.length > 0) {
      console.log('  [Debug] Sample appointment keys:', Object.keys(allItems[0]).join(', '));
      const cancelled = cancelledItems[0];
      if (cancelled) console.log('  [Debug] Sample cancelled: cancelled_at=' + cancelled.cancelled_at + ', id=' + cancelled.id);
    }

    function mapAppointment(a, forceCancel) {
      ...
      const cancelledAt = a.cancelled_at || (forceCancel ? (a.updated_at || a.created_at) : null);
      const status = cancelledAt ? 'cancelled' : deriveStatus(a);
    ...
    const rows = items.map(a => mapAppointment(a, false))
      .concat(cancelledItems.map(a => mapAppointment(a, true)));
